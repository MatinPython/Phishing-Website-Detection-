...
# def feature_extraction(url):
  ...
  def Img():
    i, internal, unsafe, external, ssl, LD = 0, 0, 0, 0, 0, 0
    for img in soup.find_all('img', src=True):
        dots = [x.start(0) for x in re.finditer('\.', img['src'])]
        if url in img['src'] or domain in img['src'] or len(dots) == 1:
            internal = internal + 1
            LD += levenshtein_distance(url, img['src'])/ max([len(url), len(img['src'])])
        elif "#" in img['src'] or "javascript" in img['src'].lower() or "mailto" in img['src'].lower() or not (url in img['src'] or domain in img['src']):
            unsafe = unsafe + 1
        else:
            external = external + 1
            LD += levenshtein_distance(url, img['src']) / max([len(url), len(img['src'])])
  ...
  def Script():
    ...
    for script in soup.find_all('script', src=True):
        ...
        elif "#" in script['src'] or "javascript" in script['src'].lower() or "mailto" in script['src'].lower() or not (url in script['src'] or domain in script['src']):
            unsafe = unsafe + 1
        else:
            external = external + 1
            LD += levenshtein_distance(url, script['src']) / max([len(url), len(script['src'])])
  ...
  def A():
    ...
    for a in soup.find_all('a', href=True):
        ...
        elif "#" in a['href'] or "javascript" in a['href'].lower() or "mailto" in a['href'].lower() or not (url in a['href'] or domain in a['href']):
            unsafe = unsafe + 1
        else:
            external = external + 1
            LD += levenshtein_distance(url, a['href']) / max([len(url), len(a['href'])])
  ...
  def Link():
    ...
    for link in soup.find_all('link', href=True):
        ...
        elif "#" in link['href'] or "javascript" in link['href'].lower() or "mailto" in link['href'].lower() or not (url in link['href'] or domain in link['href']):
            unsafe = unsafe + 1
        else:
            external = external + 1
            LD += levenshtein_distance(url, link['href']) / max([len(url), len(link['href'])])
